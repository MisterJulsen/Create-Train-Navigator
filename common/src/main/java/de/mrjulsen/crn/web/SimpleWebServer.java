...
public class SimpleWebServer {
    ...
    static class TestHandler implements HttpHandler {
        ...
        public void handle(HttpExchange t) throws IOException {
            ...
            response += "\nCRN PACK";
            for (ResourceLocation str :  ModCommonEvents.getCurrentServer().get().getPackRepository().getPack("mod:" + CreateRailwaysNavigator.MOD_ID).open().getResources(PackType.SERVER_DATA, CreateRailwaysNavigator.MOD_ID, "", (str) -> true)) {
                response += "\n - " + str;
            }
            t.sendResponseHeaders(200, response.length());
            OutputStream os = t.getResponseBody();
