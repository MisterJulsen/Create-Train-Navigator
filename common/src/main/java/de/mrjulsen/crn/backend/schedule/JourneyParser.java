...
/**
 * Parses a {@link Schedule} into an immutable {@link TrainJourney}.
 * <p>
 * The parser works purely on the schedule data, so stops, sections, train lines and categories are
 * known deterministically without waiting for the train to execute the corresponding instructions.
 */
public final class JourneyParser {
    ...
    /** Parses the train's current schedule. Returns an empty journey if it has none. */
    ...
    /**
     * Assigns every stop to its section and schedule title. Markers are collected in ascending
     * entry order, so a stop belongs to the last marker at or before it. Stops before the first
     * section marker belong to the last section, matching the cyclic execution of the schedule.
     */
    ...
    /** Whether the given journey no longer matches the train's schedule and must be re-parsed. */
