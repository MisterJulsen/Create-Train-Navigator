...
public class FlyoutTransferTimeWidget<T extends GuiEventListener & Renderable & NarratableEntry> extends AbstractFlyoutWidget<T> {

    private final MutableComponent textTransferTime = TextUtils.translate("gui." + CreateRailwaysNavigator.MOD_ID + ".search_options.transfer_time").withStyle(ChatFormatting.BOLD);
    private final UserSettings settings;

    private final Supplier<UserSetting<Integer>> getUserSetting;
    private final CreateTimeSelectionWidget timeSelection;

    public FlyoutTransferTimeWidget(DLScreen screen, FlyoutPointer pointer, ColorShade pointerShade, Consumer<T> addRenderableWidgetFunc, UserSettings settings, Supplier<UserSetting<Integer>> getUserSetting, Consumer<GuiEventListener> removeWidgetFunc) {
        super(screen, 1, 60, pointer, pointerShade, addRenderableWidgetFunc, removeWidgetFunc);
        set_width(Math.max(CreateTimeSelectionWidget.WIDHT + 16, font.width(textTransferTime) + DLIconButton.DEFAULT_BUTTON_WIDTH + 16 + 10 + FlyoutPointer.WIDTH * 2));
