...
public class JourneyCompletedPage extends AbstractRouteDetailsPage {
    ...
    public JourneyCompletedPage(JourneyTracker tracker, Runnable after) {
        super(tracker);
        this.messageLabel = MultiLineLabel.create(font, TextUtils.translate(keyAfterJourney, route().destination().displayName()), width() - 10);
        this.after = after;
    }
    ...
    public void tick() {
        ...
        if (ticks == MAX_TICK_TIME) {
            DLUtils.doIfNotNull(after, Runnable::run);
        }
    }
