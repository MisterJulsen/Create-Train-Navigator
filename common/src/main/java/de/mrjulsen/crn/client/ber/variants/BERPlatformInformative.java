...
public class BERPlatformInformative implements IBERRenderSubtype<AdvancedDisplayBlockEntity, AdvancedDisplayRenderInstance, Boolean> {
    ...
    public void update(Level level, BlockPos pos, BlockState state, AdvancedDisplayBlockEntity blockEntity, AdvancedDisplayRenderInstance parent, EUpdateReason reason) {
        ...
        */
        ...
        boolean refreshAll = reason != EUpdateReason.DATA_CHANGED ||
            !DLUtils.compareCollections(lastPredictions, preds, (a, b) -> a.stationInfo().platform().equals(b.stationInfo().platform()) && a.trainId().equals(b.trainId())) ||
    ...
    private void addLine(Level level, BlockPos pos, BlockState state, AdvancedDisplayBlockEntity blockEntity, AdvancedDisplayRenderInstance parent, EUpdateReason reason, int predictionIdx, float y) {
        ...
        // PLATFORM
        Component label = blockEntity.isPlatformFixed() ? TextUtils.empty() : TextUtils.text(lastPredictions.get(predictionIdx).stationInfo().platform());
        ...
        lastLabel = new BERText(parent.getFontUtils(), () -> {
            List<Component> texts = new ArrayList<>();
            texts.add(TextUtils.text(lastPredictions.get(predictionIdx).trainName()));
            return texts;             
        }, 0)
            .withIsCentered(false)
        ...
        lastLabel = new BERText(parent.getFontUtils(), () -> {
            List<Component> texts = new ArrayList<>();
            texts.add(TextUtils.text(lastPredictions.get(predictionIdx).scheduleTitle()));
            return texts;
        }, 0)
            .withIsCentered(false)
            .withMaxWidth(displayWidth - TIME_LABEL_WIDTH - trainNameWidth - platformWidth + 1 - SPACING, true)
            .withStretchScale(0.25f, 0.4f)
            .withStencil(0, displayWidth - TIME_LABEL_WIDTH - trainNameWidth - platformWidth + 1 - SPACING)
            .withCanScroll(true, 1)
            .withColor((0xFF << 24) | (blockEntity.getColor()))
            .withPredefinedTextTransformation(new TextTransformation(TIME_LABEL_WIDTH + trainNameWidth + SPACING, y, 0.01f, 1, 0.4f))
            .build()
        ;
    ...
    private void addNextDeparture(Level level, BlockPos pos, BlockState state, AdvancedDisplayBlockEntity blockEntity, AdvancedDisplayRenderInstance parent, EUpdateReason reason, int predictionIdx) {
        ...
        parent.labels.add(new BERText(parent.getFontUtils(), TextUtils.text(lastPredictions.get(predictionIdx).trainName()), 0)
            .withIsCentered(false)
            .withMaxWidth(timeLabelWidth, true)
        ...
        parent.labels.add(new BERText(parent.getFontUtils(), TextUtils.text(lastPredictions.get(predictionIdx).scheduleTitle()).withStyle(ChatFormatting.BOLD), 0)
            .withIsCentered(false)
