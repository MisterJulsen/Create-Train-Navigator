...
public class StationBlacklistScreen extends Screen implements IForegroundRendering {
    ...
    // Tooltips
    private final Component tooltipAdd = new TranslatableComponent("gui." + ModMain.MOD_ID + ".blacklist.add.tooltip");
    private final Component tooltipRemove = new TranslatableComponent("gui." + ModMain.MOD_ID + ".blacklist.delete.tooltip");

    @SuppressWarnings("resource")
    public StationBlacklistScreen(Level level, Screen lastScreen) {
        super(new TranslatableComponent("gui." + ModMain.MOD_ID + ".blacklist.title"));
        this.level = level;
    ...
    private void initStationDeleteButtons() {
        ...
        String[] names = GlobalSettingsManager.getInstance().getSettingsData().getBlacklist().toArray(String[]::new);
    ...
    private void addToBlacklist() {
        ...
        if (GlobalSettingsManager.getInstance().getSettingsData().isBlacklisted(value) || ClientTrainStationSnapshot.getInstance().getAllTrainStations().stream().noneMatch(x -> x.toLowerCase().equals(value.toLowerCase()))) {
            return;
        }

        GlobalSettingsManager.getInstance().getSettingsData().addToBlacklist(value, () -> {
            initStationDeleteButtons();
        });
    ...
    public void render(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) { 
        ...
        String[] blacklist = GlobalSettingsManager.getInstance().getSettingsData().getBlacklist().toArray(String[]::new);
        ...
        if (addButton.isInBounds(pMouseX, pMouseY) && !newEntryBox.getValue().isBlank()) {
            ...
            for (Entry<String, GuiAreaDefinition> entry : blacklistEntryButton.entrySet()) {
                if (entry.getValue().isInBounds(pMouseX, pMouseY + scrollOffset)) {
                    GlobalSettingsManager.getInstance().getSettingsData().removeFromBlacklist(entry.getKey(), () -> {
                        initStationDeleteButtons();
                    });
    ...
    private int getMaxScrollHeight() {
        return ENTRIES_START_Y_OFFSET + ENTRY_HEIGHT * GlobalSettingsManager.getInstance().getSettingsData().getBlacklist().size();
    }
    ...
    protected void updateEditorSubwidgets(EditBox field) {
		...
		destinationSuggestions = new ModStationSuggestions(minecraft, this, field, minecraft.font, getViableStations(field), field.getHeight() + 2 + field.y);
