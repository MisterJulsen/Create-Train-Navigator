...
public class RouteDetailsScreen extends CommonScreen implements IJourneyListenerClient {
    ...
    protected void init() {
        ...
        addTooltip(DragonLibTooltip.of(textSaveRoute).assignedTo(saveButton));
    }
    ...
    private int renderRouteStart(GuiGraphics graphics, int x, int y, StationEntry stop) {
        final int HEIGHT = 30;
        final int V = 48;

        GuiUtils.blit(Constants.GUI_WIDGETS, graphics, x, y, 0, V, ENTRY_WIDTH, HEIGHT);
        ...
        if (stop.shouldRenderRealtime() && stop.reachable(false)) {
            ...
            graphics.drawString(shadowlessFont, TimeUtils.parseTime((int)(stop.getEstimatedTimeWithThreshold() % 24000 + Constants.TIME_SHIFT), ModClientConfig.TIME_FORMAT.get()), (int)((x + ENTRY_TIME_X) / scale), pY + 10, stop.getDifferenceTime() > ModClientConfig.DEVIATION_THRESHOLD.get() ? Constants.COLOR_DELAYED : Constants.COLOR_ON_TIME);
        }
        graphics.drawString(shadowlessFont, text.getFirst(), (int)((x + ENTRY_TIME_X) / scale), pY, 0xFFFFFF);
        graphics.pose().popPose();

        Component platformText = Utils.text(stop.getUpdatedInfo().platform());
        graphics.drawString(shadowlessFont, platformText, x + ENTRY_DEST_X + 129 - shadowlessFont.width(platformText), y + 15, stop.stationInfoChanged() ? Constants.COLOR_DELAYED : 0xFFFFFF);
        ...
        graphics.drawString(shadowlessFont, name, x + ENTRY_DEST_X, y + 15, 0xFFFFFF);

        return HEIGHT;
    }

    private int renderTrainDetails(GuiGraphics graphics, int x, int y, SimpleRoutePart part) {
        final int HEIGHT = 43;
        final int V = 99;
        ...
        GuiUtils.blit(Constants.GUI_WIDGETS, graphics, x, y, 0, V, ENTRY_WIDTH, HEIGHT);
        part.getTrainIcon().render(TrainIconType.ENGINE, graphics, x + ENTRY_DEST_X, y + 7);

        graphics.pose().pushPose();
        graphics.pose().scale(scale, scale, scale);
        graphics.drawString( shadowlessFont, String.format("%s (%s)", part.getTrainName(), part.getTrainID().toString().split("-")[0]), (int)((x + ENTRY_DEST_X + 24) / scale), (int)((y + 7) / scale), 0xDBDBDB);
        graphics.drawString( shadowlessFont, String.format("→ %s", part.getScheduleTitle()), (int)((x + ENTRY_DEST_X + 24) / scale), (int)((y + 17) / scale), 0xDBDBDB);
        graphics.pose().scale(mul, mul, mul);
        graphics.pose().popPose();

        return HEIGHT;
    }

    private int renderStop(GuiGraphics graphics, int x, int y, StationEntry stop) {
        final int HEIGHT = 21;
        final int V = 78;
        ...
        Component platformText = Utils.text(stop.getUpdatedInfo().platform());
        graphics.drawString(shadowlessFont, platformText, x + ENTRY_DEST_X + 129 - shadowlessFont.width(platformText), y + 6, stop.stationInfoChanged() ? Constants.COLOR_DELAYED : 0xFFFFFF);
        ...
        graphics.drawString(shadowlessFont, name, x + ENTRY_DEST_X, y + 6, 0xFFFFFF);

        return HEIGHT;
    }

    private int renderRouteEnd(GuiGraphics graphics, int x, int y, StationEntry stop) {
        final int HEIGHT = 44;
        final int V = 142;
        ...
        Component platformText = Utils.text(stop.getUpdatedInfo().platform());
        graphics.drawString(shadowlessFont, platformText, x + ENTRY_DEST_X + 129 - shadowlessFont.width(platformText), y + 21, stop.stationInfoChanged() ? Constants.COLOR_DELAYED : 0xFFFFFF);
        ...
        graphics.drawString(shadowlessFont, name, x + ENTRY_DEST_X, y + 21, 0xFFFFFF);

        return HEIGHT;
    }

    private void renderHeadline(GuiGraphics graphics, int pMouseX, int pMouseY) {
        ...
        graphics.drawCenteredString(font, titleInfo, guiLeft + GUI_WIDTH / 2, guiTop + 19, 0xFFFFFF);
        graphics.pose().pushPose();
        graphics.pose().scale(2, 2, 2);
        graphics.drawCenteredString(font, headline, (guiLeft + GUI_WIDTH / 2) / 2, (guiTop + 31) / 2, 0xFFFFFF);
        graphics.pose().popPose();
    }

    private int renderTransfer(GuiGraphics graphics, int x, int y, long a, long b, StationEntry nextStation) {
        final int HEIGHT = 24;
        final int V = 186;
        ...
        if (nextStation != null && !nextStation.reachable(true)) {
            if (nextStation.isTrainCanceled()) {                
                ...
                graphics.drawString(shadowlessFont, textTrainCanceled, x + ENTRY_TIME_X + ModGuiIcons.ICON_SIZE + 4, y + 8, 0xFFFFFF);
            } else if (nextStation.isDeparted()) {                
                ModGuiIcons.CROSS.render(graphics, x + ENTRY_TIME_X, y + 13 - ModGuiIcons.ICON_SIZE / 2);
                graphics.drawString(shadowlessFont, textConnectionMissed, x + ENTRY_TIME_X + ModGuiIcons.ICON_SIZE + 4, y + 8, 0xFFFFFF);
            } else {
                ModGuiIcons.WARN.render(graphics, x + ENTRY_TIME_X, y + 13 - ModGuiIcons.ICON_SIZE / 2);
                graphics.drawString(shadowlessFont, textConnectionEndangered, x + ENTRY_TIME_X + ModGuiIcons.ICON_SIZE + 4, y + 8, 0xFFFFFF);
            }
        } else {
            graphics.drawString(shadowlessFont, transferText.getString() + " " + (time < 0 ? "" : "(" + TimeUtils.parseDuration((int)time) + ")"), x + ENTRY_TIME_X, y + 8, 0xFFFFFF);
        }
    ...
    public void renderBg(GuiGraphics graphics, int pMouseX, int pMouseY, float pPartialTick) { 
        ...
        int yOffs = guiTop + 45;
        GuiUtils.blit(Constants.GUI_WIDGETS, graphics, guiLeft + 16, yOffs, 22, ENTRIES_START_Y_OFFSET, 0, 48, 22, 1, 256, 256);
        ...
        for (int i = 0; i < partsArray.length; i++) {
            ...
            if (btn.active) {
                ...
                btn.render(graphics, pMouseX, (int)(pMouseY - scrollOffset), pPartialTick);
            }
        ...
        GuiUtils.blit(Constants.GUI_WIDGETS, graphics, guiLeft + 16, yOffs, 22, AREA_H, 0, 48, 22, 1, 256, 256);
        graphics.pose().popPose();
